...
select jobdepartment.jobdept,round(avg(salarybonus.amount),0) as avg_salary
from  salarybonus 
join jobdepartment  on jobdepartment.job_id=salarybonus.job_id
...
SELECT JobDept,
    ROUND( AVG(
            ( CAST( TRIM(REPLACE(SUBSTRING_INDEX(salaryrange, '-', 1), '$', ''))AS UNSIGNED)
			+CAST( TRIM(REPLACE(SUBSTRING_INDEX(salaryrange, '-', -1), '$', ''))AS UNSIGNED
			)) / 2
    ), 2) AS Avg_Salary
FROM jobdepartment
...
select jd.JobDept as JOB_dept,sb.amount as highest_sal
 from salarybonus sb
 join jobdepartment jd on jd.job_id=sb.job_id
 order by highest_sal desc
 limit 1;
...
select count(emp_id) as No_qual 
from qualification;
...
-- How do leave days correlate with payroll amounts?
 ...
 COUNT(l.Leave_ID) AS LeaveDays,p.Total_Amount AS PayrollAmount
 FROM Employee e
